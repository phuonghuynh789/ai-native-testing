...
**[Replaced 2026-08-22]** The transition-based query above is gone entirely (`buildReadyForTestJql` deleted from `sprint-report-jql.ts`). Ready for Test Tickets/SP is no longer its own Jira search — it's now the exact same population as the Root Cause Tickets Trễ sub-table's "Ready for Testing or In Test Tickets": the Committed set (Sprint-field scoped, see above), filtered by *current* status being Ready for Testing or In Test (`filterReadyOrInTest()` in `sprint-report-delivery.ts`, shared by both `computeSandboxDateBreakdown` and `sprint-report-service.ts`). This guarantees the two counts always match — they were previously computed two different ways (transition-during-window vs. current-status-of-Committed) and could disagree. The corresponding Jira link (`deliveryJiraLinks.readyForTest`) changed the same way, to `buildCommittedJql + status in ("Ready for Testing", "In Test") + scope` (the same JQL `sandboxDateJiraLinks.readyOrInTest` already used) — a `buildReadyOrInTestJql(sprintCode, scope)` helper in `sprint-report-jira-links.ts` is now shared by both links. Impact Analysis Review's population followed along too, by design: `impactAnalysisJiraLinks` and the keyword-check input (`rowReadyForTest` in `sprint-report-service.ts`) both now derive from this same filtered set, so `buildImpactAnalysisJiraLinks` dropped its `dateParams` argument in favor of `sprintCode`. Net effect: one fewer Jira search per refresh (4 instead of 5), and Ready for Test/Impact Analysis now reflect a live snapshot rather than a historical transition count.
...
- Auto: **[Reversed 2026-08-22]** Delivery, Quality, and Impact Analysis are now pre-selected per row by `suggestExecutiveSummary()` (`packages/web/src/executiveSummarySuggestions.ts`), and Overall is derived from whichever of those three have data. This reverses the original "no invented numeric threshold" design call — the user explicitly asked for a self-assessment based on the computed results, so the thresholds below were confirmed with them rather than invented unilaterally:
  ...
  - **Overall**: `bad` if any of the three (that have data) is `bad`; `good` only if every one that has data is `good`; `medium` otherwise (covers `partial`, mixed good/no-data, or no indicator having data at all — in which case Overall is left `unset` too).
