...
- Rows are exactly `['PC', 'PCFUM', 'PCPOP_MP', 'PCPOP_UO', 'PCPOP_RC']`. `PC`/`PCFUM` map directly from the issue's `project.key`. `PCPOP` issues are split by the `Product Domain` custom field: `Merchant Platform`→`PCPOP_MP`, `Customer Experience`→`PCPOP_UO`, `Reconciliation Core`→`PCPOP_RC`. A `PCPOP` issue with no recognized Product Domain value is dropped (not counted in any row). **[Updated 2026-08-20]** Row key renamed `PCPOP_CE` → `PCPOP_UO` (cosmetic only, matching the squad's real short name) — the Task 4/5 code blocks below still show the pre-rename key. The `Customer Experience` domain string itself, shown in those same code blocks, was briefly changed to `User Operation` on a hunch during dogfooding, then reverted back to `Customer Experience` after direct confirmation from a real Jira lookup — the code blocks below already match the current real source on this point.
